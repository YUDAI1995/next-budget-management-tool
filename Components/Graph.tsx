...
export const Graph: React.FC<GraphProps> = (props) => {
  const data = {
    ...
    datasets: [
      {
        label: "Dataset",
        data: [sumAmount(props.moneyList, 0), sumAmount(props.moneyList, 1)],
        backgroundColor: [balanceType[0].color, balanceType[1].color],
        borderColor: [balanceType[0].borderColor, balanceType[1].borderColor],
        borderWidth: 1,
      },
    ],
  };
    ...
    <section className="graphSection">
      ...
      <div
        className={classNames("doughnutWrapper", {
          noData:
            sumAmount(props.moneyList, 0) + sumAmount(props.moneyList, 1) === 0,
        })}
      >
        {sumAmount(props.moneyList, 0) + sumAmount(props.moneyList, 1) !== 0 ? (
          <Doughnut data={data} width={240} height={240} 
          //options={options}
          />
        ) : (
          "データがありません"
